...
public class mm1queue {
    ...
    ArrayList<Double> cpt;			// list of monitoring checkpoint times (cpt)
    int numChecks;					// number of cpt 
    ...
    public mm1queue(double lambda, double ts, double endTime) {
        this.lambda = lambda;
        this.ts = ts;
    ...
    public void birthGen(double time) {
     ...
     */
    public void deathGen() {
    	schedule.pop(); // pop the completed request out of the queue 
    	if (!schedule.isEmpty()) { // if there is a pending request, modify its contents to be a death event 
    		Event next = schedule.pop();
    		deathSchedule(next.getTime());     		        		
    	} else{ 
     ...
     * system.
     */
    public void deathSchedule(double arrivalTime) {
    ...
    public void monitor(PrintWriter out) {
    	...
    	q += cur_q;
    	cpt.remove(0);
    	out.println("Check-point time: " + clock);
    ...
    public void run(PrintWriter out) {
    	while (clock < 2*endTime) {
    		if (cpt.get(0) < nextArrival && cpt.get(0) < nextDeparture) {
    			clock = cpt.get(0);
    ...
    public void printStats(PrintWriter out) {
    	...
    	System.out.println("w = " + w/numChecks + "  requests");
    	System.out.println("q = " + q/numChecks + "  requests");
    	System.out.println("Tw = " + Tw/requests + " sec");
    	System.out.println("Tq = " + Tq/serviced + "  sec");
    	System.out.println("Ts = " + Ts/serviced+ "  sec");
    }
}